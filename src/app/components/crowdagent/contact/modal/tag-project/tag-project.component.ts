...
export class TagProjectComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log('tag-project.currentlySelectedTags=', this.currentlySelectedTags)
  ...
  isChecked(tag: string): boolean {
    ...
    if (this.currentlySelectedTags) {
      result = this.currentlySelectedTags.find(entry => entry.tag == tag) != undefined
    }
    return result
  }
  ...
  checkboxChanged(event: any): void {
    ...
    let entry: number = -1
    if (this.currentlySelectedTags) {
      entry = this.currentlySelectedTags.findIndex(entry => entry.tag ? entry.tag === tagName : false)
    }
