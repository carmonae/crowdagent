...
function sort(titles: PiquedTitlesType[], column: SortColumn, direction: string): PiquedTitlesType[] {
	if (direction === '' || column === '') {
		return titles;
	} else {
...
function matches(titleData: PiquedTitlesType, term: string, pipe: PipeTransform) {
	return (
		titleData.title.toLowerCase().includes(term.toLowerCase()) ||
    titleData.subtitle.toLowerCase().includes(term.toLowerCase()) ||
    pipe.transform(titleData.score).includes(term) 
	);
}
...
export class PiquedTitleTablesService {
    ...
    private _search(): Observable<SearchResult> {
      ...
      const { sortColumn, sortDirection, pageSize, page, searchTerm, thedata } = this._state;

      // 1. sort      
      let piQueing = sort(thedata, sortColumn, sortDirection);
  
      // 2. filter
      piQueing = piQueing.filter((pique) => matches(pique, searchTerm, this.pipe));
      const total = piQueing.length;
  
      // 3. paginate
      piQueing = piQueing.slice((page - 1) * pageSize, (page - 1) * pageSize + pageSize);
